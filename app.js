(async function () {
  ...
  const APP_VERSION = "20260612-v12";
  ...
  const state = {
    ...
    bulkHighlightIds: new Set(),
    bulkHighlightLabelsVisible: true,
    selectedId: null,
    highlightedId: null,
  ...
  const el = {
    ...
    bulkHighlightInput: document.getElementById("bulkHighlightInput"),
  ...
  async function loadLazyPointData() {
    try {
      ...
      if (
        (!state.currentDrawingId || !drawings.some(function(drawing) { return drawing.id === state.currentDrawingId; })) &&
        typeof manifest.currentDrawingId === "string" &&
        manifest.currentDrawingId &&
        drawings.some(function(drawing) { return drawing.id === manifest.currentDrawingId; })
      ) {
  ...
  function codePrefix(value) {
    ...
    if (!str) return "";
    var digits = str.match(/\d{4}/);
    if (digits) return digits[0];
    var alpha = str.match(/^[A-Za-z]+/);
  ...
  function syncAutoGroupsForDrawing(drawingId) {
    ...
    state.groups.forEach(function(group) {
      ...
      var prefixes = new Set(items.map(function(annotation) {
        return codePrefix(annotation.code);
      }).filter(Boolean));
      if (prefixes.size === 1) {
    ...
    state.annotations.forEach(function(annotation) {
      ...
      if (annotation.special) return; // Cabinets and modules keep their manual group
  ...
  function annotationMatchesPrefix(annotation, prefix) {
    return Boolean(prefix) && codePrefix(annotation && annotation.code) === prefix;
  }
  ...
  function selectedRenderPrefixes() {
    ...
    var legacyPrefix = codePrefix(state.renderPrefix);
  ...
  function clearPlcFilter() {
    state.renderPrefixes = new Set();
    state.renderPrefix = "";
    state.activeGroupId = "";
    state.selectedId = null;
    state.highlightedId = null;
    state.bulkHighlightIds.clear();
    renderDrawingList();
    renderOverlay();
    renderMinimapList();
    updateEditor();
    ...
    if (hasRenderPrefixes()) {
      return annotations.filter(function(annotation) {
        return annotationMatchesAnyRenderPrefix(annotation) ||
          ...
          state.bulkHighlightIds.has(annotation.id) ||
          annotationIsDuplicate(annotation);
      });
    }
    if (isCompactViewport()) {
      return annotations.filter(function(annotation) {
        return annotation.id === state.selectedId ||
          annotation.id === state.highlightedId ||
          state.bulkHighlightIds.has(annotation.id);
      });
    }
  ...
  function renderMobilePlcList() {
    ...
    allButton.dataset.action = "clear-plc-filter";
    ...
    Array.from(byPrefix.entries()).sort(function(a, b) {
      ...
      var responsibility = responsibilityForPrefix(prefix);
      var button = document.createElement("button");
      button.type = "button";
      button.className = "mobile-plc-chip";
      var teamClass = responsibilityClassName(responsibility);
      if (teamClass) button.classList.add(teamClass);
      button.classList.toggle("active", selectedPrefixes.indexOf(prefix) !== -1);
      button.title = responsibility ? responsibility.team + (responsibility.location ? " / " + responsibility.location : "") : "";
      button.innerHTML = "<strong>" + escapeHtml(groupDisplayName(group) || prefix) + "</strong><small>" +
        count + (responsibility ? " " + escapeHtml(responsibility.team.replace("分队", "")) : "") + "</small>";
    ...
    if (selectedPrefixes.length) {
      var selectedAnnotations = annotations.filter(function(annotation) {
        return selectedPrefixes.indexOf(codePrefix(annotation.code)) !== -1;
      }).sort(function(a, b) {
        return compareDeviceCodes(annotationTitle(a), annotationTitle(b));
      });
      var pointRow = document.createElement("div");
      pointRow.className = "mobile-plc-point-chip-row";
      selectedAnnotations.forEach(function(annotation) {
        var responsibility = responsibilityForAnnotation(annotation);
        var codeButton = document.createElement("button");
        codeButton.type = "button";
        codeButton.className = "mobile-plc-point-chip";
        var teamClass = responsibilityClassName(responsibility);
        if (teamClass) codeButton.classList.add(teamClass);
        ...
        pointRow.appendChild(codeButton);
      });
      el.mobilePlcList.appendChild(pointRow);
    }
  }

  function renderPrefixForAnnotation(annotation) {
    return codePrefix(annotation && annotation.code);
  }
  ...
  function groupSortLabel(groupId, annotations) {
    ...
    if (annotations && annotations.length) return codePrefix(annotations[0].code) || annotationTitle(annotations[0]);
  ...
  function responsibilityForPrefix(prefix) {
    var key = firstFourDigits(prefix);
    return key ? responsibilityInfo.items[key] || null : null;
  }

  function responsibilityForAnnotation(annotation) {
    return responsibilityForPrefix(annotation && annotation.code);
  }

  function responsibilityClassName(responsibility) {
    return responsibility && responsibility.className ? "responsibility-" + responsibility.className : "";
  }

  function responsibilityFieldHtml(responsibility) {
    if (!responsibility) return "";
    var fields = [
      { label: "责任分队", value: responsibility.team },
      { label: "责任区域", value: responsibility.location || "" }
    ].filter(function(field) { return field.value; });
    return fields.map(function(field) {
      return "<div class=\"responsibility-field\"><dt>" + escapeHtml(field.label) + "</dt><dd>" + escapeHtml(field.value) + "</dd></div>";
    }).join("");
  }
  ...
  async function loadResponsibilityZones() {
    try {
      ...
      responsibilityInfo.teams = payload && payload.teams && typeof payload.teams === "object" ? payload.teams : {};
      responsibilityInfo.loaded = true;
      renderDrawingList();
      renderMobilePlcList();
      renderOverlay();
      ...
      console.warn("Responsibility zones load failed:", error.message);
      return false;
    }
  }
  ...
  function renderDeviceInfoInto(panel, annotation, compact) {
    ...
    if (!deviceInfo.loaded) {
      ...
      panel.innerHTML =
        "<div class=\"device-info-head\"><strong>" + escapeHtml(annotationTitle(annotation)) + "</strong><span>" +
        escapeHtml(responsibility ? responsibility.team : "设备清单") + "</span></div>" +
        (responsibility ? "<dl class=\"device-info-fields responsibility-fields\">" + responsibilityFieldHtml(responsibility) + "</dl>" : "") +
    ...
    if (!info) {
      ...
      panel.innerHTML =
        "<div class=\"device-info-head\"><strong>" + escapeHtml(code) + "</strong><span>" +
        escapeHtml(responsibility ? responsibility.team : "无清单匹配") + "</span></div>" +
        (responsibility ? "<dl class=\"device-info-fields responsibility-fields\">" + responsibilityFieldHtml(responsibility) + "</dl>" : "") +
        "<p class=\"device-info-empty\">设备总清单中没有完全匹配的点位编号。</p>";
      return;
    }

    var fields = Array.isArray(info.fields) ? info.fields.slice() : [];
    if (responsibility && !fields.some(function(field) { return field.label === "责任分队"; })) {
      fields.unshift({ label: "责任区域", value: responsibility.location || "" });
      fields.unshift({ label: "责任分队", value: responsibility.team });
    }
    var primary = fields.filter(function(field) {
      return ["责任分队", "责任区域", "设备名称", "规格/型号", "长度（mm）", "宽度（mm)", "速度v(m/sec)", "电机功率P(W)", "标称电流In(A)", "制动", "变频/软启", "皮带类型"].indexOf(field.label) >= 0;
    });
  ...
  function batchPrefixSet() {
    return new Set(state.batchCodes.map(codePrefix).filter(Boolean));
  }

  function annotationIsBatchExtra(annotation) {
    var normalized = normalizeDeviceCode(annotation && annotation.code);
    if (!normalized || !state.batchCodes.length) return false;
    var prefix = codePrefix(normalized);
  ...
  function toPointAnnotation(annotation) {
    ...
    return {
      ...annotation,
      type: "point",
      special: Boolean(annotation.special),
  ...
  function switchDrawing(drawingId, options = {}) {
    ...
    updateBatchCodePanel();
    updateEditor();
    if (lazyPoints.enabled && !lazyPoints.loadedDrawingIds.has(drawingId)) {
      loadLazyDrawing(drawingId).then(function(loaded) {
        if (!loaded || state.currentDrawingId !== drawingId) return;
        syncAutoGroupsForDrawing(drawingId);
        renderDrawingList();
        renderOverlay();
        renderMinimapList();
  ...
  function renderOverlay(options = {}) {
    ...
    state.bulkHighlightIds.forEach(function(id) { visibleIds.add(id); });
  ...
  function renderMinimapList() {
    ...
    for (const [groupId, groupedAnnotations] of orderedGroupEntries) {
      ...
      var rowTeamClass = responsibilityClassName(responsibilityForAnnotation(annotation));
      if (rowTeamClass) row.classList.add(rowTeamClass);
  ...
  function drawAnnotation(annotation) {
    ...
    const isBulkHighlighted = state.bulkHighlightIds.has(annotation.id);
    const isHighlighted = annotation.id === state.highlightedId || isBulkHighlighted;
    const isSelected = annotation.id === state.selectedId;
    const isCompactFocus = isCompactViewport() && isHighlighted;
    const responsibility = responsibilityForAnnotation(annotation);
    ...
    var teamClass = responsibilityClassName(responsibility);
    if (teamClass) shape.classList.add(teamClass);
    ...
    const shouldShowLabel = annotation.code && (
      state.labelsVisible ||
      (!isBulkHighlighted && (isSelected || annotation.id === state.highlightedId)) ||
      (isBulkHighlighted && state.bulkHighlightLabelsVisible)
    );
    if (shouldShowLabel) {
      const label = document.createElementNS(SVG_NS, "text");
  ...
  function createAnnotation(type, imagePoints) {
    ...
    var autoGroup = ensureAutoGroup(codePrefix(code), state.currentDrawingId);
  ...
  async function search() {
    ...
    // Only apply prefix filter for short queries without dots (prefix-style search)
    // Full codes like "3808.00.99" search notes, not prefix filter
    if (query.indexOf(".") === -1) {
      const queryPrefix = codePrefix(query);
  ...
  function drawingTitleById(drawingId) {
    var drawing = drawings.find(function(item) { return item.id === drawingId; });
    return drawing ? drawing.title : drawingId || "";
  }

  function bulkHighlightDrawingRank(drawingId) {
    if (drawingId === state.currentDrawingId) return -1;
    var fixedIndex = FIXED_DRAWING_ORDER.indexOf(drawingId);
    if (fixedIndex !== -1) return fixedIndex;
    var drawingIndex = drawings.findIndex(function(item) { return item.id === drawingId; });
    return drawingIndex === -1 ? Number.MAX_SAFE_INTEGER : drawingIndex;
  }

  function bulkHighlightSpatialSort(items) {
    var groups = new Map();
    items.forEach(function(item) {
      var drawingId = item.annotation && item.annotation.drawingId || "";
      if (!groups.has(drawingId)) groups.set(drawingId, []);
      groups.get(drawingId).push(item);
    });

    var ordered = [];
    Array.from(groups.entries()).sort(function(a, b) {
      var rankA = bulkHighlightDrawingRank(a[0]);
      var rankB = bulkHighlightDrawingRank(b[0]);
      if (rankA !== rankB) return rankA - rankB;
      return String(drawingTitleById(a[0])).localeCompare(String(drawingTitleById(b[0])), "zh-CN", { numeric: true });
    }).forEach(function(entry) {
      var remaining = entry[1].slice().map(function(item) {
        return {
          item: item,
          center: annotationCenter(item.annotation)
        };
      });
      remaining.sort(function(a, b) {
        if (Math.abs(a.center.y - b.center.y) > 80) return a.center.y - b.center.y;
      ...
      while (current) {
        ...
        for (var i = 0; i < remaining.length; i++) {
          var dx = remaining[i].center.x - current.center.x;
          var dy = remaining[i].center.y - current.center.y;
  ...
  async function bulkHighlightPoints() {
    if (!el.bulkHighlightInput || !el.bulkHighlightResults) return;
    var codes = parseBulkHighlightCodes(el.bulkHighlightInput.value);
    el.bulkHighlightResults.innerHTML = "";
    state.bulkHighlightIds.clear();
    if (!codes.length) {
      el.bulkHighlightResults.textContent = "请输入点位编号";
      renderOverlay();
      return;
    }
    ...
    codes.forEach(function(code) {
      var found = searchableAnnotations.filter(function(annotation) {
        return normalizeDeviceCode(annotation.code) === code;
      });
      if (found.length) {
        found.forEach(function(annotation) { matches.push({ code: code, annotation: annotation }); });
      } else {
        missing.push(code);
      }
    });

    var drawingIds = Array.from(new Set(matches.map(function(item) {
      return item.annotation.drawingId;
    }).filter(Boolean)));
    if (lazyPoints.enabled) {
      for (var i = 0; i < drawingIds.length; i++) {
        await loadLazyDrawing(drawingIds[i]);
    ...
    var hydratedMatches = matches.map(function(item) {
      var full = state.annotations.find(function(annotation) { return annotation.id === item.annotation.id; });
      return { code: item.code, annotation: full || item.annotation };
    });
    hydratedMatches = bulkHighlightSpatialSort(hydratedMatches);
    hydratedMatches.forEach(function(item) {
      if (item.annotation && item.annotation.id) state.bulkHighlightIds.add(item.annotation.id);
    });

    var summary = document.createElement("div");
    summary.className = "bulk-highlight-summary";
    summary.textContent = "已匹配 " + hydratedMatches.length + " 个，未找到 " + missing.length + " 个；已按楼层和相邻位置排序";
    el.bulkHighlightResults.appendChild(summary);
    ...
    hydratedMatches.forEach(function(item) {
      ...
      button.innerHTML =
        ...
        (annotation.note ? " / " + escapeHtml(annotation.note) : "") +
        "</small>";
      button.addEventListener("click", function() {
        focusSearchMatch(annotation);
      });
      el.bulkHighlightResults.appendChild(button);
    });

    if (missing.length) {
      var missingEl = document.createElement("div");
      missingEl.className = "bulk-highlight-missing";
      missingEl.textContent = "未找到：" + missing.join("、");
      el.bulkHighlightResults.appendChild(missingEl);
    }

    var currentMatches = hydratedMatches.filter(function(item) {
      return item.annotation.drawingId === state.currentDrawingId;
    });
    var first = currentMatches[0] || hydratedMatches[0];
    if (first && first.annotation) {
      if (first.annotation.drawingId !== state.currentDrawingId) {
        switchDrawing(first.annotation.drawingId, { selectedId: first.annotation.id, highlightedId: first.annotation.id });
      } else {
        state.selectedId = first.annotation.id;
        state.highlightedId = first.annotation.id;
        zoomToAnnotation(first.annotation);
      }
    }
    renderDrawingList();
    renderOverlay();
  ...
  function bindEvents() {
    ...
    if (el.mobilePlcList) {
      el.mobilePlcList.addEventListener("click", function(event) {
        var target = event.target instanceof Element ? event.target.closest("[data-action='clear-plc-filter']") : null;
        if (!target || !el.mobilePlcList.contains(target)) return;
        event.preventDefault();
        event.stopPropagation();
        clearPlcFilter();
      });
    ...
    if (el.bulkHighlightButton) {
      el.bulkHighlightButton.addEventListener("click", bulkHighlightPoints);
    }
    if (el.bulkHighlightLabels) {
      el.bulkHighlightLabels.checked = state.bulkHighlightLabelsVisible;
      el.bulkHighlightLabels.addEventListener("change", function() {
        state.bulkHighlightLabelsVisible = el.bulkHighlightLabels.checked;
        renderOverlay();
      });
    }
    if (el.bulkHighlightInput) {
      el.bulkHighlightInput.addEventListener("keydown", function(event) {
  ...
  function startManifestSync() {
    const intervalMs = isCompactViewport() ? 30000 : 10000;
